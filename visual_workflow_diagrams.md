...
graph TD
    %% Doctor Nodes
    Doctor([Therapist / Doctor]) --> Auth[Login / Verification]
    Auth --> Dashboard{Therapist Dashboard}
    
    %% Management Path
    Dashboard -->|Manage Time| Calendar[Update Calendar Availability]
    Dashboard -->|Pending Requests| Review[Review Patient Bookings]
    Review --> Approve[Approve / Reject Appointment]
    Approve --> Notify[System sends Push Notification to Patient]
    
    %% Session Path
    Dashboard -->|Join Meeting| WebRTC[Start WebRTC Video Call]
    WebRTC --> LiveTools[Use Live Tools: Transcript & Chat]
    
    %% Review Path
    Dashboard -->|Post-Session| Analytics[View Earnings & Analytics]
